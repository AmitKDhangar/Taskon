// Element Quering
const UpdatesContainer = document.body.querySelector("ul") as HTMLElement;
// interface
...
// global object
let notificationHolder: InotificationDetail = {
  id: 0,
  tasktitle: "",
  taskdes: "",
  taskassignee: "",
...
let notificationStore: object[] = [];
// signatures
let notificationDetail: (notificDetail: InotificationDetail) => void;
// data on DOMload
...
notificationStore.forEach((notify, idx) => {
  ...
  notification.innerHTML = `<div class = "updates">
                     <img src="https://cdn-icons-png.freepik.com/256/6319/6319720.png?semt=ais_white_label">
                   <div class="titleanddescription">
                  <p class="updates-title">${notify.tasktitle}</p>
                  <p class="updates-description">${notify.taskdes}</p>
                  </div>
                  </div>`;
  notification.dataset.index = idx.toString();
  UpdatesContainer.appendChild(notification);
  const notificationlist = document.body.querySelectorAll(
    "li"
...
notificationDetail = (notificDetail: InotificationDetail) => {
  const notificContainer = document.body.querySelector(
    ".detail-updates"
  ) as HTMLElement;
...
let linkStore: object = {};
// add btn
const addBtn = document.body.querySelector(".ri-add-line") as HTMLElement;
// savechangesbtn
const updateBtn = document.body.querySelector(".Updatebtn") as HTMLElement;
// linkfield
const linkField = document.body.querySelector("input") as HTMLInputElement;
// linkcontainer
const linkContainer = document.body.querySelector(".links") as HTMLElement;
// Event Listener
